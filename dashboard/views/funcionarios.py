...
url = os.getenv('ARQUIVO_BASE_SALARIO_FUNCIONARIOS_CORUPA')
resp = requests.get(url)
resp.raise_for_status()
csv_text = resp.text

df = pd.read_csv(io.StringIO(csv_text), encoding='Latin1')
...
with tab1:
    ...
    # pega a data de hoje
    data_ref = pd.to_datetime(f'01-05-{datetime.today().year}', dayfirst=True)
    df_data_ref = df[df['data'] == data_ref]

    # Métrica para comaparar o mês anterior ao atual
    mes_anterior = data_ref.month - 1
    if data_ref.month == 1:
        mes_anterior = 12

    data_mes_anterior = pd.to_datetime(f'01-{mes_anterior}-{data_ref.year}', dayfirst=True)
